# Cougar Mountain Alloy Robotics Website

This is the website for Cougar Mountain Alloy Robotics - an alliance of FIRST Robotics teams based at Cougar Mountain Middle School in Washington State. It is designed to introduce our teams, celebrate student accomplishments, recognize sponsors, share photos, and make it easy for families, students, sponsors, and community members to connect with the program. 

Students work together to research problems, design innovative solutions, build and program LEGO robots, and compete in robotics challenges. Through FIRST Robotics, students develop skills in:

* Engineering and design
* Coding and robotics
* Teamwork and communication
* Problem-solving and iteration
* Research and innovation
* Gracious Professionalism and Coopertition

## Why "Alloy" And Not Our School Name?

An alloy is multiple metals combined together for strength. Just like an alloy, we are made up of multiple FIRST Robotics teams - each one unique, but stronger together. Located on Cougar Mountain, we chose the name Cougar Mountain Alloy Robotics Teams to reflect both where we come from and what we believe: that collaboration makes us all stronger.

## About the Site

The website includes sections for:

* **Our Teams:** Information about the FLL teams in the program
* **Awards:** Past accomplishments and recognition
* **Photo Gallery:** Team photos, events, competitions, build days, and behind-the-scenes moments
* **Sponsors:** Organizations and people who support the teams
* **Contact:** Ways to connect with our robotics program
